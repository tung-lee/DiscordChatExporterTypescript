...
// Map from emoji character to name code
const emojiToCode = new Map<string, string>([
  ['😀', 'grinning'],
  ['😃', 'smiley'],
  ['😄', 'smile'],
  ['😁', 'grin'],
...
// Map from name code to emoji character (includes aliases)
...
/**
 * Get all emoji characters from the index (for building the standard emoji regex)
 * Returns emoji sorted by length descending (longest first) to handle compound emoji
 */
export function getAllEmoji(): string[] {
  return Array.from(emojiToCode.keys()).sort((a, b) => b.length - a.length);
}
...
export const emojiIndex = {
  /**
   * Get emoji code name from character
   */
  getCode(emoji: string): string | null {
    return emojiToCode.get(emoji) ?? null;
  },

  /**
   * Get emoji character from code name
   */
  getByName(name: string): string | null {
    return codeToEmoji.get(name) ?? null;
  },
};
